...
import { ProcessOptions, processTranslationFile } from './process-translation-file';

import { compileTranslations } from './compile-translations';

import { getTranslationFiles } from './get-translation-files';
...
const run = ({
    ...
    if (compiledTranslations) {
        compiledTranslations.forEach((file: string) =>
            processTranslationFile(path.resolve(cwd, file), write, processOptions),
        );
